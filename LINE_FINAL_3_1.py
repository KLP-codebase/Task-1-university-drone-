...
def set_mode_guided(m):
    ...
    # Use the high-level set_mode_apm function first
    try:
        m.set_mode_apm('GUIDED')
    except Exception:
        # Fallback: Send DO_SET_MODE command directly
        # For ArduCopter, GUIDED is custom_mode 4
...
def is_armed(m):
    # Request a fresh heartbeat to check current state
...
def arm(m, force=False):
    ...
    # Force arming if standard arming fails (21196 is the "magic" force parameter for ArduPilot)
...
def rotate_90_clockwise(m, duration_s=4.0):
    ...
    m.mav.command_long_send(
        ...
        0, 90.0, angular_speed, 1.0, 0.0, 0, 0, 0
    )
...
def land(m):
    ...
    try:
        ...
        # Fallback to custom mode 9 for LAND
        m.mav.command_long_send(
            m.target_system, m.target_component,
            mavutil.mavlink.MAV_CMD_DO_SET_MODE, 0,
            1, 9, 0, 0, 0, 0, 0
