    ...
    if not force:
        try:
            ...
            while True:
                ...
                if ack and ack.command == mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM: break
                if not ack: break
...
def process_frame(frame, speed_base, last_scanned_id, force_hover=False):
    gray = frame.copy()
    h, w = gray.shape
    ...
    if len(detections) > 0:
        ...
        # Centering logic used for both Scanning and Landing
        if tag_id != last_scanned_id or force_hover:
            ...
            if abs(dx) < tol and abs(dy) < tol:
                vx, vy = 0.0, 0.0
                cv2.putText(display_frame, "CENTERED / LOCKED", (10, 70), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            else:
                vx = np.clip(-dy * 0.002, -0.2, 0.2)
                vy = np.clip(dx * 0.002, -0.2, 0.2)
                cv2.putText(display_frame, "ALIGNING", (10, 70), 
    ...
    if not tag_active and not force_hover:
        ...
        if lines is not None:
            ...
            # Drawing the detected Hough lines in green
            for line in lines:
                x1, y1, x2, y2 = line[0]
...
def _camera_and_control_loop(host, port, stop_event, m, speed_base):
    ...
    try:
        while not stop_event.is_set():
            ...
            # If in landing mode, we force the vision to stay centered on the tag
            vx, vy, yaw_rate, res_img, tag_data, tag_active = process_frame(frame, speed_base, last_scanned_id, force_hover=landing_mode)
            
            vz = 0.0
            
            if landing_mode:
                # Precision Descent Logic
                # If we are centered (vx and vy are 0), descend slowly
                if vx == 0 and vy == 0:
                    vz = 0.25 # Move DOWN in Body NED
                    cv2.putText(res_img, "DESCENDING ON TAG", (10, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 0, 255), 2)
                
                # Check altitude to trigger final land
                msg = m.recv_match(type='GLOBAL_POSITION_INT', blocking=False)
                if msg:
                    curr_alt = msg.relative_alt / 1000.0
                    if curr_alt < 0.3: # Close to ground
                        print("Close to ground. Cutting motors.")
                        land(m)
                        stop_event.set()
                ...
                if time.time() - scan_timer > 1.5:
                    ...
                    if tags_scanned_count == 1:
                        ...
                        print("Second tag scanned. Entering Precision Landing Mode.")
                        landing_mode = True
            else:
...
def main():
    ...
    set_mode_guided(m)
    if not arm(m): return
    if not takeoff(m, args.alt): land(m); return
