...
# Note: Ensure 'apriltag' or 'pupil-apriltag' is installed via pip
...
def process_frame(frame, speed_base, last_scanned_id):
    ...
    Returns: vx, vy, yaw_rate, display_frame, current_tag_info, is_tag_active
    """
    # Ensure frame is contiguous to prevent crashes in Python 3.12 C-extensions
    gray = frame.copy()
    h, w = gray.shape
    ...
    # 1. AprilTag Detection (using apriltag library)
    detections = TAG_DETECTOR.detect(gray)
    tag_active = False
    tag_info = None # Stores (id, hamming)

    if len(detections) > 0:
        # Sort by proximity to center if multiple tags exist
        ...
        if tag_id != last_scanned_id:
            ...
            tol = 40 # pixel tolerance for "center"
            dx = tag_cx - center_x
            dy = tag_cy - center_y
            
            if abs(dx) < tol and abs(dy) < tol:
                vx, vy, yaw_rate = 0.0, 0.0, 0.0 # Stop to scan
                cv2.putText(display_frame, f"LOCK: ID {tag_id} (H:{hamming})", (10, 70), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            else:
                # P-Controller for centering
                vx = -dy * 0.005 # Note: In downward camera -dy is move forward
                vy = dx * 0.005
                cv2.putText(display_frame, "ALIGNING OVER TAG", (10, 70), 
...
def _camera_and_control_loop(host, port, stop_event, m, speed_base):
    ...
    try:
        while not stop_event.is_set():
            ...
            # Logic: vx, vy, yaw, image, (id, hamming), active
            vx, vy, yaw_rate, res_img, tag_data, tag_active = process_frame(frame, speed_base, last_scanned_id)
            
            # Handle Tag Scanning Timer & Hamming Verification
            if tag_active and vx == 0 and vy == 0: 
                if scan_timer is None:
                    scan_timer = time.time()
                
                # Hold for 1.5 seconds to ensure stable reading
                if time.time() - scan_timer > 1.5:
                    tag_id, hamming = tag_data
                    # Verification via Hamming: 0 is perfect, lower is better.
                    status = "VERIFIED" if hamming == 0 else f"UNCERTAIN (Hamming: {hamming})"
