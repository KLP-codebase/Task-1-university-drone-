...
MAVLink Line Follower & AprilTag Mission:
 ...
 - Simultaneously follow lines and scan AprilTags using 'dt_apriltags'
...
# Initialize the high-performance dt_apriltags detector
...
def process_frame(frame, speed_base, last_scanned_id):
    ...
    Returns: vx, vy, yaw_rate, display_frame, tag_info, is_tag_active
    """
    gray = frame.copy() # Contingous copy for C-extension stability
    h, w = gray.shape
    vx, vy, yaw_rate = 0.0, 0.0, 0.0
    display_frame = cv2.cvtColor(gray, cv2.COLOR_GRAY2BGR)
    center_x, center_y = w // 2, h // 2
    
    # 1. AprilTag Detection (dt_apriltags)
    ...
    if len(detections) > 0:
        ...
        if tag_id != last_scanned_id:
            ...
            if abs(dx) < tol and abs(dy) < tol:
                ...
                vx = -dy * 0.005 # Center P-gain
                vy = dx * 0.005
                cv2.putText(display_frame, "CENTERING OVER TAG", (10, 70), 
    ...
    if not tag_active:
        ...
        if lines is not None:
            ...
            cv2.putText(display_frame, "PATH TRACKING", (10, 35), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        else:
            cv2.putText(display_frame, "LOST PATH", (10, 35), 
...
def _camera_and_control_loop(host, port, stop_event, m, speed_base):
    try:
        ...
        print(f'Socket: Connected to stream on {host}:{port}')
    except Exception as e:
        ...
        while not stop_event.is_set():
            ...
            if tag_active and vx == 0 and vy == 0: 
                ...
                if time.time() - scan_timer > 2.0:
                    ...
                    status = "VALID" if hamming == 0 else f"UNCERTAIN (H:{hamming})"
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] SCAN COMPLETE: ID {tag_id} ({status})")
