...
MAVLink Line Follower & AprilTag Mission:
 ...
 - POST-MISSION: Executes ./stop.sh automatically upon disarm.
"""
import time
import argparse
import threading
...
def rotate_90_clockwise(m, duration_s=5.0):
    global is_rotating
    print(f'Action: Rotating 90 degrees clockwise (relative)')
    is_rotating = True
...
def run_stop_script():
    """Executes the ./stop.sh command in the terminal."""
    script_path = "./stop.sh"
    if os.path.exists(script_path):
        print(f"Executing {script_path}...")
        try:
            # Use subprocess to run the script in a new shell
...
def main():
    ...
    # Final safety check: if we disarmed via keyboard interrupt, run stop logic
    if is_armed(m): 
        land(m)
    else:
        run_stop_script()
