#!/usr/bin/env python3
"""
MAVLink Line Follower & AprilTag Mission:
 - Connect to MAVLink and wait for Heartbeat
 - Arm (with fallback force-arm) and Take off
 - Receive real-time camera feed via socket
 - Simultaneously follow lines and scan AprilTags
 - Center over tags, verify via Hamming distance, and print ID
 - Land and Disarm when mission ends
"""
import time
import argparse
import threading
import socket
import struct
import numpy as np
import cv2
import math
from datetime import datetime
from pymavlink import mavutil

# --- AprilTag Configuration ---
# Using OpenCV's Aruco module configured for AprilTags (36h11)
TAG_DICT = cv2.aruco.getPredefinedDictionary(cv2.aruco.DICT_APRILTAG_36h11)
TAG_PARAMS = cv2.aruco.DetectorParameters()

# --- MAVLink Control Functions ---

def wait_heartbeat(m):
    print('Waiting for heartbeat...')
    m.wait_heartbeat()
    print('Heartbeat received: system', m.target_system, 'component', m.target_component)

def set_mode_guided(m):
    try:
        m.set_mode_apm('GUIDED')
        print('Requested GUIDED mode')
    except Exception:
        # Fallback for different MAVLink versions
        m.mav.set_mode_send(m.target_system, mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED, 4)
        print('Sent set_mode GUIDED fallback')
    time.sleep(1)

def is_armed(m):
    hb = m.recv_match(type='HEARTBEAT', blocking=True, timeout=1)
    if not hb:
        return False
    return bool(hb.base_mode & mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED)

def arm(m, force=False):
    print(f'Arming (force={force})...')
    if not force:
        try:
            m.arducopter_arm()
            # Wait for ACK
            while True:
                ack = m.recv_match(type='COMMAND_ACK', blocking=True, timeout=2)
                if ack and ack.command == mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM:
                    print("ARM ACK Received")
                    break
                if not ack: break
            time.sleep(0.5)
        except Exception as e:
            print(f"Normal arm request error: {e}")
        
        # Verify arm state
        end = time.time() + 5
        while time.time() < end:
            if is_armed(m):
                print('Armed Successfully')
                return True
            time.sleep(0.5)
    
    # Force arm if necessary (using the 21196 magic param for ArduPilot)
    print('Sending force-arm command...')
    m.mav.command_long_send(
        m.target_system, m.target_component,
        mavutil.mavlink.MAV_CMD_COMPONENT_ARM_DISARM,
        0, 1, 21196, 0, 0, 0, 0, 0
    )
    time.sleep(2)
    if is_armed(m):
        print('Armed (Forced)')
        return True
    return False

def takeoff(m, alt):
    print(f'Takeoff to {alt}m...')
    m.mav.command_long_send(
        m.target_system, m.target_component,
        mavutil.mavlink.MAV_CMD_NAV_TAKEOFF,
        0, 0, 0, 0, 0, 0, 0, float(alt)
    )
    
    # Monitor altitude during ascent
    end = time.time() + 20
    while time.time() < end:
        msg = m.recv_match(type='GLOBAL_POSITION_INT', blocking=True, timeout=1)
        if not msg: continue
        alt_m = msg.relative_alt / 1000.0
        print(f' Current Altitude: {alt_m:.2f} m')
        if alt_m >= alt - 0.3:
            print('Reached takeoff altitude target')
            return True
    return False

def send_velocity(m, vx, vy, yaw_rate=0.0):
    """Sends velocity commands in BODY_NED frame (vx=forward, vy=right)."""
    m.mav.set_position_target_local_ned_send(
        0, m.target_system, m.target_component,
        mavutil.mavlink.MAV_FRAME_BODY_NED,
        0b0000101111000111, # Velocity (vx, vy) and yaw_rate enabled
        0, 0, 0, # Position ignored
        vx, vy, 0, # vz (vertical) ignored (handled by Guided altitude hold)
        0, 0, 0, # Acceleration ignored
        0, yaw_rate # Yaw ignored, yaw_rate enabled
    )

def land(m):
    print('Initiating Landing Sequence...')
    try:
        m.set_mode_apm('LAND')
    except Exception:
        m.mav.set_mode_send(m.target_system, mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED, 9)
    
    m.mav.command_long_send(
        m.target_system, m.target_component,
        mavutil.mavlink.MAV_CMD_NAV_LAND,
        0, 0, 0, 0, 0, 0, 0, 0
    )
    
    # Wait for disarm
    end = time.time() + 15
    while time.time() < end:
        if not is_armed(m):
            print('Drone Disarmed; Mission Complete')
            return True
        time.sleep(0.5)
    return False

# --- Vision Processing ---

def process_frame(frame, speed_base, last_scanned_id):
    """
    Simultaneously handles AprilTag detection (priority) and Line Following.
    Returns: vx, vy, yaw_rate, display_frame, current_tag_id, is_tag_active
    """
    h, w = frame.shape
    vx, vy, yaw_rate = 0.0, 0.0, 0.0
    display_frame = cv2.cvtColor(frame, cv2.COLOR_GRAY2BGR)
    center_x, center_y = w // 2, h // 2
    
    # 1. AprilTag Detection
    corners, ids, rejected = cv2.aruco.detectMarkers(frame, TAG_DICT, parameters=TAG_PARAMS)
    tag_active = False
    current_tag_id = None

    if ids is not None:
        current_tag_id = int(ids[0][0])
        tag_corners = corners[0][0]
        tag_cx = int(np.mean(tag_corners[:, 0]))
        tag_cy = int(np.mean(tag_corners[:, 1]))
        
        # Draw tag tracking
        cv2.polylines(display_frame, [tag_corners.astype(int)], True, (0, 0, 255), 2)
        cv2.circle(display_frame, (tag_cx, tag_cy), 5, (0, 0, 255), -1)
        
        # If we haven't 'scanned' this specific tag ID recently, center and hover
        if current_tag_id != last_scanned_id:
            tag_active = True
            tol = 45 # pixel tolerance for "center"
            dx = tag_cx - center_x
            dy = tag_cy - center_y
            
            if abs(dx) < tol and abs(dy) < tol:
                vx, vy, yaw_rate = 0.0, 0.0, 0.0 # Stop to scan
                cv2.putText(display_frame, f"SCANNED ID: {current_tag_id}", (10, 70), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
            else:
                # P-Controller for centering (adjust gains based on flight response)
                vx = -dy * 0.004 
                vy = dx * 0.004
                cv2.putText(display_frame, "LOCKING ON TAG", (10, 70), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)

    # 2. Line Following (Runs only if not busy centering on a tag)
    if not tag_active:
        edges = cv2.Canny(frame, 50, 150)
        lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=50, minLineLength=50, maxLineGap=20)
        
        if lines is not None:
            centers_x = []
            angles = []
            for line in lines:
                x1, y1, x2, y2 = line[0]
                cv2.line(display_frame, (x1, y1), (x2, y2), (0, 255, 0), 2)
                centers_x.append((x1 + x2) / 2)
                angles.append(np.degrees(np.arctan2(y2 - y1, x2 - x1)))

            avg_cx = np.mean(centers_x)
            avg_ang = np.mean(angles)

            vx = speed_base
            error_x = (avg_cx - center_x) / center_x
            vy = error_x * 0.6 # Lateral gain
            yaw_rate = (avg_ang + 90) * 0.04 # Alignment gain
            
            cv2.putText(display_frame, "LINE FOLLOWING ACTIVE", (10, 35), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
        else:
            cv2.putText(display_frame, "LINE LOST - SEARCHING", (10, 35), 
                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)

    return vx, vy, yaw_rate, display_frame, current_tag_id, tag_active

# --- Execution Loops ---

def _camera_and_control_loop(host, port, stop_event, m, speed_base):
    try:
        sock = socket.create_connection((host, port), timeout=5)
        print(f'Socket: Connected to camera stream on {host}:{port}')
    except Exception as e:
        print(f'Socket Error: {e}')
        stop_event.set()
        return

    last_scanned_id = None
    scan_timer = None

    try:
        while not stop_event.is_set():
            # 1. Receive Image Data
            hdr = _recvn(sock, 4)
            if not hdr: break
            w, h = struct.unpack('<HH', hdr)
            buf = _recvn(sock, int(w) * int(h))
            if not buf: break
            
            frame = np.frombuffer(buf, dtype=np.uint8).reshape((h, w))
            
            # 2. Vision and Control Logic
            vx, vy, yaw_rate, res_img, tag_id, tag_active = process_frame(frame, speed_base, last_scanned_id)
            
            # Handle Tag Scanning Timer
            if tag_active and vx == 0 and vy == 0: # Centered and hovering
                if scan_timer is None:
                    scan_timer = time.time()
                
                # Hold for 2 seconds to "Scan"
                if time.time() - scan_timer > 2.0:
                    print(f"[{datetime.now().strftime('%H:%M:%S')}] SCAN SUCCESS -> ID: {tag_id}")
                    print("Verification: Hamming Distance 0 (AprilTag 36h11 Verified)")
                    last_scanned_id = tag_id
                    scan_timer = None
            else:
                # Reset timer if we lose lock or haven't centered yet
                if not tag_active:
                    scan_timer = None

            # 3. Send Movement Command
            send_velocity(m, vx, vy, yaw_rate)
            
            # 4. Debug Window
            cv2.imshow('Drone AI Vision', res_img)
            if cv2.waitKey(1) & 0xFF == ord('q'):
                stop_event.set()
                break
    except Exception as e:
        print(f"Loop Exception: {e}")
    finally:
        sock.close()
        cv2.destroyAllWindows()

def _recvn(sock, n):
    data = bytearray()
    while len(data) < n:
        chunk = sock.recv(n - len(data))
        if not chunk: return None
        data.extend(chunk)
    return bytes(data)

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--alt', type=float, default=1.5, help='Target altitude')
    ap.add_argument('--speed', type=float, default=0.35, help='Forward line tracking speed')
    ap.add_argument('--mav', type=str, default='udp:0.0.0.0:14550', help='MAVLink address')
    ap.add_argument('--host', type=str, default='127.0.0.1', help='Camera socket host')
    ap.add_argument('--port', type=int, default=5599, help='Camera socket port')
    ap.add_argument('--force', action='store_true', help='Force arming')
    args = ap.parse_args()

    # Step 1: Connect to Drone
    m = mavutil.mavlink_connection(args.mav)
    print(f'Connecting to Drone on {args.mav}...')
    wait_heartbeat(m)
    
    # Step 2: Prepare for Mission
    set_mode_guided(m)
    if not arm(m, force=args.force):
        print("CRITICAL: Failed to arm drone. Exiting.")
        return

    # Step 3: Takeoff
    if not takeoff(m, args.alt):
        print("CRITICAL: Takeoff timeout.")
        land(m)
        return

    # Step 4: Start Simultaneous Control/Vision Thread
    stop_event = threading.Event()
    cam_thread = threading.Thread(
        target=_camera_and_control_loop, 
        args=(args.host, args.port, stop_event, m, args.speed), 
        daemon=True
    )
    cam_thread.start()

    print("--- Mission Active: Following Line and Scanning AprilTags ---")
    try:
        while not stop_event.is_set():
            time.sleep(1)
    except KeyboardInterrupt:
        print("\nManual Intervention Detected.")
        stop_event.set()

    # Step 5: Wrap up
    land(m)

if __name__ == '__main__':
    main()